...
module IB
  ...
  class Flex
    ...
    def run(return_raw = false)
      # Initiate FLEX request at a known FLEX Web Service URI
      raw_resp = get_content Flex.uri, :t => @token, :q => @query_id, :v => 3
      resp = XmlSimple.xml_in(raw_resp, :ForceArray => false)
      ...
      report = nil
      until report do
        raw_report = get_content(report_uri, :t => @token, :q => reference_code, :v => 3,
                               :text_ok => @format != :xml)
        report = XmlSimple.xml_in(raw_report, :ForceArray => false)
